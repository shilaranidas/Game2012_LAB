...
void createModel()
{
	...
	{
		//this line gives me circle, orthographic projection can't see z, so its view like one circle
		//when we change projection from ortho to frustum, then it look like circle spiner circle inside circle connected
		//like coil
		//vertices[i] = glm::vec3(R * cos(t), R * sin(t), t - 60.0);
		//now im changing y as t. here every cicle draw upper of previous circle, spiral
...
void display(void)
{
	...
	//it go to more near so, bigger, some truncate top and bottom
	//projection = glm::frustum(-5.0, 5.0, -5.0, 5.0, 10.0, 100.0); //move the front face back
	//it go to less near so, smaller, some truncate top and bottom
