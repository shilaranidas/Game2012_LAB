...
// DynamicPolygon.cpp
//
// This program draws a line loop/TRIANLG FAN with vertices equally apart on 
...
void createModel1(int n)
{
	for (int i = 0; i < n; ++i)
	{

		vertices[i] = glm::vec3(X + R * cos(theta), Y + R * sin(theta), 1.0);
		colors[i] = glm::vec3((float)rand() / (float)RAND_MAX, (float)rand() / (float)RAND_MAX, (float)rand() / (float)RAND_MAX);
...
void display(void)
{
	...
	transformObject(1.0f, XY_AXIS, rotAngle += 1.0f, glm::vec3(0.0f, 0.0f, 0.0f));
