...
void display(void)
{
	glClearColor(0.5f, 0.5, 0.5, 255);
	glClear(GL_COLOR_BUFFER_BIT | GL_DEPTH_BUFFER_BIT);

	glBindVertexArray(vao);

	transformObject(0.5f, YZ_AXIS, (float)counter++ / 90.0f, glm::vec3(0.0f, 0.0f, 0.0f));

	//Ordering the GPU to start the pipeline
	//glDrawElements(GL_QUADS,15, GL_UNSIGNED_SHORT, 0);
	//glDrawElements(GL_LINE_LOOP, 15, GL_UNSIGNED_SHORT, 0);
